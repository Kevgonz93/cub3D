...
/*   Updated: 2026/02/04 19:02:33 by kegonzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	padding_line(t_game *game, char **new_grid, int new_width)
{
	int	old_height;
	int	i;
...
void	paddeding_map(t_game *game)
{
	...
	new_height = game->config.map->height + 2;
	new_grid = NULL;
	new_grid = (char **)malloc(sizeof(char *) * (new_height + 1));
	if (!new_grid)
		return ;
	if (padding_line(game, new_grid, new_width))
	{
		free_array(new_grid);
		return ;
	}

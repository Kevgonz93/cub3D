...
double	get_angle(t_game *game, int x)
{
	...
	initial_fov_angle = player_angle - (game->config.fov / 2);
...
void	pre_dda(t_game *game, double ray_angle, t_dda *dda)
{
...
void	run_dda(t_game *game, t_dda *dda)
{
	dda->hit = 0;
	while (dda->hit == 0)
	{
...
void	paint_walls(t_game *game)
{
	...
	while (x < 3 * HEIGHT / 4)
	{
		my_put_pixel(&game->img, WIDTH / 2 - 1, x, 0xFF0000);
		x++;
	}
}

double	get_dist_hit(t_game *game, t_dda *dda)
{
	double	dist_hit;
...
double	correct_dist(double dist_hit, double ray_angle, t_game *game)
{
	double	player_angle;
	double	dist;

	player_angle = atan2(game->player.dir_y, game->player.dir_x);
	dist = dist_hit * cos(ray_angle - player_angle);
	if (dist < 0.00001)
		dist = 0.00001;
	return (dist);
}
...
void	render_walls(t_game *game)
{
	...
	while (x < WIDTH)
	{
		...
		run_dda(game, &dda);
		dist_hit = get_dist_hit(game, &dda);
		dist_corrected = correct_dist(dist_hit, ray_angle, game);
		// calcular distancia + altura de pared + dibujar columna x
		x++;
	}
}
